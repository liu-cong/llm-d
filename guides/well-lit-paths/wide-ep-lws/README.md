...
```bash
cd guides/well-lit-paths/wide-ep-lws/
```
...
Deploy the Gateway and HTTPRoute using the [gateway recipe](../../primitives/gateway/README.md).
...
As with PD, the `wide-ep-lws` guide supports selective PD. For information on this refer to [this section of the PD docs](../../well-lit-paths/pd-disaggregation/README.md#tuning-selective-pd).
...
2. Follow the [benchmark guide](../../../guides/supporting/benchmark/README.md) to deploy the benchmark tool and analyze the benchmark results. Notably, select the corresponding benchmark template:
...
```bash
# From guides/well-lit-paths/wide-ep-lws
